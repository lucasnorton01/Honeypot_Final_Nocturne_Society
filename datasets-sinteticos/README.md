...
El tag `Honeypot_Cowrie` preserva un directorio `analitica/` que no fue incluido en la nueva rama main (creada desde `2147ad4` como root commit). Ese directorio contenía derivados generados a partir de este corpus sintético, no evidencia del pipeline:

- `analitica/honeypot.db` — base SQLite derivada de `postgres-dump-20260811.sql` (generada por `generar_sqlite.py`)
- `analitica/kpis.json` — KPIs recalculados desde la base local (editado a mano el 2026-08-18)
- `analitica/tablas/*.csv` — tablas de análisis numérico
- `analitica/parquet/*.parquet` — datasets Parquet para Databricks
- `analitica/consultas.sql` — consultas documentadas

**No son evidencia del pipeline**: nunca pasaron por event-ingest → ioc-extractor → report-generator. Son artefactos derivados del corpus sintético, generados offline.
