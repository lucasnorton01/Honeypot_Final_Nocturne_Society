#!/usr/bin/env node
/*
 * generar_dataset.js - Dataset SINTETICO de dimensionamiento (30 dias, 201.125 eventos). NO es evidencia.
 * Nocturne Society - Honeypots + n8n - Tesis de grado - 2026
 *
 * Genera archivos sinteticos de la ventana 2026-07-13 a 2026-08-11 (UTC-3):
 *   datasets-sinteticos/postgres-dump-20260811.sql   (events, iocs, reports, error_log)
 *   datasets-sinteticos/cowrie-events.json           (157.234 eventos Cowrie, con los 13 reales verbatim)
 *   datasets-sinteticos/dionaea-events.json          (43.891 eventos Dionaea)
 *   datasets-sinteticos/report-YYYYMMDD.json         (30 reportes diarios)
 *   datasets-sinteticos/geoip-mapping.json           (3.128 IPs publicas -> pais)
 *   datasets-sinteticos/resumen-dataset.json         (agregados)
 *
...
// Pico de actividad a las 04:00 local (UTC-3), madrugada
...
// ======================= 9. Escritura =======================
const ROOT = path.resolve(__dirname, '..'), EVID = path.join(ROOT, 'datasets-sinteticos');
...
function escribirDump(rows) {
  ...
  const L = [];
  const hdr = [
    '-- DATASET SINTETICO generado por generar_dataset.js. NO es un volcado de la base de datos real ni evidencia.',
    '--',
  ...
  L.push('', '--', '-- Dataset sintetico completo', '--');
