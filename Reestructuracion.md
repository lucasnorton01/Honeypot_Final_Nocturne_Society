...
| Archivo | Contenido |
|---------|-----------|
| `postgres-cred.json` | Contraseña `honeypot_pass` en texto plano |
| `n8n/workflows/postgres-credential.json` | Contraseña `honeypot_pass` en texto plano |
...
| `POSTGRES_PASSWORD` | `honeypot_pass` | `contrasena_ejemplo_cambiar` |
| `POSTGRES_DB` | `honeypot` | `nombre_db_ejemplo` |

### Advertencia

El historial de git sigue conteniendo la contraseña `honeypot_pass` en texto plano (commits previos). Se recomienda rotar la contraseña y usar `git filter-repo` si el repositorio es público.
