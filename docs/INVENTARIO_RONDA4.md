...
| Archivo | Línea | Campo |
|---------|-------|-------|
| `postgres-cred.json` | 12 | `"password": "honeypot_pass"` |
| `n8n/workflows/postgres-credential.json` | 11 | `"password": "honeypot_pass"` |
| `Pasos para levantar proyecto.txt` | 2 | `password:Norton01` (credencial de n8n) |
| `Pasos para levantar proyecto.txt` | 3 | `password:test123` (credencial de Cowrie — esta es la credencial legítima del honeypot, no un secreto) |
| `.env.example` | 9 | `POSTGRES_PASSWORD=honeypot_pass` (valor de ejemplo, aceptable) |
...
3. **`Pasos para levantar proyecto.txt`** contiene credenciales en texto plano (n8n: nortonlucas01@gmail.com / Norton01)
